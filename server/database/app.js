...
app.get('/fetchDealers/:state', async (req, res) => {
    try {
      const dealers = await Dealerships.find({ state: new RegExp(`^${req.params.state}$`, "i") });
      if (dealers.length > 0) {
        res.json(dealers);
      } else {
...
app.get('/fetchDealer/:id', async (req, res) => {
    try {
      const dealer = await Dealerships.findOne({ id: parseInt(req.params.id) }); // Buscamos por el campo `id`
      if (dealer) {
        res.json(dealer);
      } else {
